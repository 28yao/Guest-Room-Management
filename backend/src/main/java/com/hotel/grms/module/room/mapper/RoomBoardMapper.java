...
public interface RoomBoardMapper {
    ...
    @Select("<script>"
            ...
            + "CASE WHEN EXISTS (SELECT 1 FROM stay_order so WHERE so.room_id = r.id "
            + "AND so.status = 'IN_HOUSE' "
            + "AND COALESCE(so.check_in_at, CONCAT(so.arrival_date, ' 18:00:00')) &lt; #{viewDateEnd} "
            + "AND CONCAT(so.departure_date, ' 12:00:00') &gt; #{viewDateStart}) "
            + "THEN 1 ELSE 0 END AS in_house_on_view_date, "
            + "CASE WHEN EXISTS (SELECT 1 FROM reservation res WHERE res.room_id = r.id "
            + "AND res.status IN ('CONFIRMED','PENDING') "
            ...
            + "CASE WHEN EXISTS (SELECT 1 FROM stay_order so WHERE so.room_id = r.id "
            + "AND so.status = 'IN_HOUSE') THEN 1 ELSE 0 END AS has_active_stay, "
            + "CASE WHEN EXISTS (SELECT 1 FROM reservation res WHERE res.room_id = r.id "
            + "AND res.status IN ('CONFIRMED','PENDING') AND res.departure_at &gt; NOW()) "
            + "THEN 1 ELSE 0 END AS has_active_reservation "
