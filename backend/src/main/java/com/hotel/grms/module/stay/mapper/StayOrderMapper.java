...
public interface StayOrderMapper extends BaseMapper<StayOrder> {
     ...
     */
    @Select("<script>"
            + "SELECT s.id AS stay_id, s.stay_no, s.reservation_id, s.room_id, s.room_type_id, "
            ...
            + "f.id AS folio_id, f.total_amount AS folio_total_amount, f.paid_amount AS folio_paid_amount, res.res_no "
            + "FROM stay_order s "
            ...
            + "AND (g.guest_name LIKE CONCAT('%', #{guestName}, '%') OR s.guest_name LIKE CONCAT('%', #{guestName}, '%')) "
            + "</if>"
            + "ORDER BY s.check_in_at DESC"
            + "</script>")
    List<StayInHouseRow> selectInHouseRows(@Param("guestName") String guestName);
