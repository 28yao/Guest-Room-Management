...
| 最后更新 | 2026-05-22（MOD-HK + MOD-SHIFT 结班） |
...
**代码现状**：MOD-INFRA～MOD-SHIFT 首批已完成；下一模块 **MOD-STAT** → **MOD-AUDIT** → **T-QA**。

### 近期交付摘要（MOD-HK + MOD-SHIFT）

- **保洁**：`GET /hk/tasks`、`POST /hk/tasks/{id}/complete`；退房/置脏自动生成 `hk_task`；前端 `/housekeeping`；`hk01` 仅保洁菜单（V16 权限隔离）
- **交班**：`GET /shifts/{id}/handover-preview`、`POST /shifts/{id}/close`；现金/微信/支付宝汇总；待办（在住/待扫/未释放预订）阻断结班；`shift:force_close` 可强制结班；前端 `/shift`
...
| 数据库与工程骨架（T-INFRA） | 已完成 | V1～V16 迁移脚本（旧库见 README §数据库） |
...
| 环境 | 旧库缺列导致 50002 | README §数据库：V3～V16 |
| 业务 | OQ-01～05 已在 plan 锁定 | 变更先改 spec |
| 质量 | 房态并发与超售 | 乐观锁 + 可售校验（预订已接入） |

## 下次更新时间

**MOD-STAT 统计页落地后**，或 **2026-05-28** 例行同步。
